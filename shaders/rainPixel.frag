...
float rainDist(vec2 p, float scale, float intensity) {
	...
	// shift Y
	p.y += 0.0 * mod(ix, 2.0) * 0.5 + (rand(vec2(ix)) - 0.5) * 0.3;
	float iy = floor(p.y);
...
void main() {
	...
	for (int i = 0; i < numLayers; i++) {
		...
		if (r < 0.0) {
			...
			wpos.x += v * 10.0 * uScale * 1.0;
			wpos.y -= v * 2.0 * uScale * 1.0;

			wpos.x += v * 10.0 * 0.0 * 1.0;
			wpos.y -= v * 2.0 * 0.0 * 1.0;
	...
	*/
	...
	color += add;
	color = mix(color, uRainColor*0.5, 0.1 * rainSum);
